...
def _parse_korean_contributors(
    author_raw: str,
) -> tuple[list[str], list[str], list[str], list[str], list[str], dict[str, str]]:
    ...
    if not author_raw.strip():
        return authors, translators, compilers, illustrators, designers, foreign_author_originals
    ...
    for segment in segments:
        ...
        elif re.search(r"엮음|엮은이|편저|엮고", role):
            # 예: "홍길동 (엮음)"
    ...
    if not authors and not translators and not compilers and not illustrators and not designers:
...
def fetch_book_info_aladin(isbn: str, ttb_key: str) -> BookInfo:
    ...
    authors, translators, compilers, illustrators, designers, foreign_author_originals = _parse_korean_contributors(author_raw)
    if authors:
...
def _bookinfo_from_nl_raw(raw: dict, isbn: str) -> BookInfo:
    ...
    author_raw = (raw.get("AUTHOR") or "").strip()
    authors, translators, compilers, illustrators, designers, _ = _parse_korean_contributors(author_raw)
...
def _build_245_responsibility(book: BookInfo) -> str:
    ...
    if authors:
        ...
        # 사용자 요청: 첫 저자 표시에 "지음"을 함께 넣기
        if not re.search(r"(지음|지은이|저자|글)\s*$", first_author):
            first_author = f"{first_author} 지음"
        chunks.append(f"/$d{_escape_marc_subfield_data(first_author)}")
        coauthors = authors[1:]
        if coauthors:
            for i, name in enumerate(coauthors):
                suffix = " 공저" if i == len(coauthors) - 1 else ""
                chunks.append(
                    f",$e{_escape_marc_subfield_data(name)}{suffix}",
                )

    for name in book.translators or []:
        chunks.append(f";$e{_escape_marc_subfield_data(name)} 옮김")

    for name in book.compilers or []:
        chunks.append(f";$e{_escape_marc_subfield_data(name)} 엮음")

    for name in book.illustrators or []:
        chunks.append(f";$e{_escape_marc_subfield_data(name)} 그림")

    for name in book.designers or []:
        chunks.append(f";$e{_escape_marc_subfield_data(name)} 디자인")
